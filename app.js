...
function treeAndArray() {
  ...
  document.querySelector('#instructions').innerHTML = "Node can have maximum 2 children.";
  if (inputText.value !== '') {
...
function createBinarySearchTree() {
  ...
  if (inputText.value !== '') {
    ...
    document.querySelector('#instructions').innerHTML = "The input data sorted and arranged into a Binary Search Tree. The left subtree of a node contains only nodes with keys lesser than the nodes key. The right subtree of a node contains only nodes with keys greater than the nodes key.";
    let tree = new Tree();
